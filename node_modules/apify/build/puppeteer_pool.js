...
const events_1 = require("./session_pool/events");
...
class PuppeteerInstance {
    constructor(options) {
        const { id, sessionPool, proxyConfiguration, launchPuppeteerFunction, } = options;
        this.id = id;
        this.proxyInfo = null;
        this.session = null;
        this.activePages = 0;
...
class PuppeteerPool {
    ...
    constructor(options = {}) {
        ow_1.default(options, ow_1.default.object.exactShape({
            maxOpenPagesPerInstance: ow_1.default.optional.number,
            retireInstanceAfterRequestCount: ow_1.default.optional.number,
            launchPuppeteerFunction: ow_1.default.optional.function,
            puppeteerOperationTimeoutSecs: ow_1.default.optional.number,
            instanceKillerIntervalSecs: ow_1.default.optional.number,
            killInstanceAfterSecs: ow_1.default.optional.number,
            launchPuppeteerOptions: ow_1.default.optional.object,
            recycleDiskCache: ow_1.default.optional.boolean,
            ...
            useLiveView: ow_1.default.optional.boolean,
            sessionPool: ow_1.default.optional.object,
        ...
        maxOpenPagesPerInstance = 50, retireInstanceAfterRequestCount = 100, launchPuppeteerFunction = (launchPuppeteerOptions) => puppeteer_1.launchPuppeteer(launchPuppeteerOptions), puppeteerOperationTimeoutSecs = 15, instanceKillerIntervalSecs = 60, killInstanceAfterSecs = 300, launchPuppeteerOptions, recycleDiskCache = false, useIncognitoPages = false, proxyConfiguration, useLiveView = false, sessionPool, log = utils_log_1.default, } = options;
        ...
        this.proxyConfiguration = proxyConfiguration;
        this.liveViewServer = useLiveView ? new live_view_server_1.default() : null;
        ...
        this.launchPuppeteerFunction = async ({ proxyUrl }) => {
            ...
            if (opts.timeout == null)
                opts.timeout = this.puppeteerOperationTimeoutMillis;
        ...
        if (sessionPool) {
            ...
            sessionPool.on(events_1.default.SESSION_RETIRED, this._retireBrowserWithSession);
        }
    }
    /**
    ...
    async _initBrowser(instance) {
        ...
        browser.on('disconnected', () => {
            ...
            if (!instance.killed)
                this.log.error('Puppeteer sent "disconnect" event. Maybe it crashed???', { id });
    ...
    _retireInstance(instance) {
        ...
        if (!this.activeInstances[id])
            return this.log.debug('browser is retired already', { id });
    ...
    async _killInstance(instance) {
        ...
        const recycleDiskCache = () => {
            if (!instance.recycleDiskCacheDir)
                return;
    ...
    _incrementPageCount(instance) {
        ...
        instance.totalPages++;
        if (instance.totalPages >= this.retireInstanceAfterRequestCount)
            this._retireInstance(instance);
    }
    /**
     ...
     */
    async newPage() {
        if (this.liveViewServer && !this.liveViewServer.isRunning())
    ...
    async _openNewTab() {
        ...
        try {
            ...
            const page = await utils_1.addTimeoutToPromise(context.newPage(), this.puppeteerOperationTimeoutMillis, 'browser.newPage() timed out.');
     ...
     * @ignore
     */
    async _focusOldestTab(browser) {
        const pages = await browser.pages();
        if (pages.length > 1)
            return pages[1].bringToFront();
    }
    /**
    ...
    async destroy() {
        ...
        const closePromises = browserPromises.map(async (browserPromise) => {
            const browser = await browserPromise;
            await browser.close();
            if (browser.recycleDiskCacheDir)
                await this._deleteDiskCacheDir(browser.recycleDiskCacheDir);
        });
        try {
    ...
    async recyclePage(page) {
        ...
        if (this.reusePages) {
            ...
            try {
                await utils_1.addTimeoutToPromise(page.close(), this.puppeteerOperationTimeoutMillis, 'page.close() timed out.');
            }
            catch (err) {
    ...
    async serveLiveViewSnapshot(page) {
        ...
        if (instance.id !== this.browserCounter - 1)
            return;
