...
class RequestList {
    ...
    async _addPersistedRequests(persistedRequests) {
        ...
        const requestStream = serialization_1.createDeserialize(persistedRequests);
    ...
    async persistState() {
        ...
        if (this.isStatePersisted)
            return;
        try {
            await key_value_store_1.setValue(this.persistStateKey, this.getState());
            this.isStatePersisted = true;
        }
        catch (err) {
    ...
    async _persistRequests() {
        const serializedRequests = await serialization_1.serializeArray(this.requests);
        await key_value_store_1.setValue(this.persistRequestsKey, serializedRequests, { contentType: CONTENT_TYPE_BINARY });
        this.areRequestsPersisted = true;
    }
    /**
    ...
    async _loadStateAndPersistedRequests() {
        ...
        if (this.initialState) {
            ...
            state = key_value_store_1.getValue(this.persistStateKey);
            if (state)
                this.log.debug('Loaded state from key value store using the persistStateKey.');
        }
        if (this.persistRequestsKey) {
            persistedRequests = await key_value_store_1.getValue(this.persistRequestsKey);
            if (persistedRequests)
    ...
    async isEmpty() {
        this._ensureIsInitialized();
        return !utils_1.getFirstKey(this.reclaimed) && this.nextIndex >= this.requests.length;
    }
    /**
    ...
    async isFinished() {
        this._ensureIsInitialized();
        return !utils_1.getFirstKey(this.inProgress) && this.nextIndex >= this.requests.length;
    }
    /**
    ...
    async fetchNextRequest() {
        ...
        const uniqueKey = utils_1.getFirstKey(this.reclaimed);
        if (uniqueKey) {
 ...
 * @function
 */
exports.openRequestList = async (listName, sources, options = {}) => {
    ow_1.default(listName, ow_1.default.any(ow_1.default.string, ow_1.default.null));
    ow_1.default(sources, ow_1.default.array);
