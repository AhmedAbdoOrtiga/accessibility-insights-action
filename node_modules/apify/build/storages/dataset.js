 ...
 * @ignore
 */
exports.checkAndSerialize = (item, limitBytes, index) => {
    const s = typeof index === 'number' ? ` at index ${index} ` : ' ';
    let payload;
    try {
        ow_1.default(item, ow_1.default.object.plain);
 ...
 * @ignore
 */
exports.chunkBySize = (items, limitBytes) => {
    if (!items.length)
        return [];
    if (items.length === 1)
        return items;
    ...
    for (const payload of items) { // eslint-disable-line
        ...
        if (bytes <= limitBytes && (bytes + 2) > limitBytes) {
            ...
            if (!Array.isArray(_.last(chunks)))
...
class Dataset {
     ...
     */
    async pushData(data) {
        ow_1.default(data, ow_1.default.object);
        ...
        if (!Array.isArray(data)) {
            const payload = exports.checkAndSerialize(data, limit);
            return dispatch(payload);
        }
        // Handle Arrays
        const payloads = data.map((item, index) => exports.checkAndSerialize(item, limit, index));
        const chunks = exports.chunkBySize(payloads, limit);
    ...
    async drop() {
        ...
        const manager = new storage_manager_1.default(Dataset);
 ...
 * @function
 */
exports.openDataset = (datasetIdOrName, options = {}) => {
    ow_1.default(datasetIdOrName, ow_1.default.optional.string);
 ...
 * @function
 */
exports.pushData = async (item) => {
    const dataset = await exports.openDataset();
