...
const injectFile = async (page, filePath, options = {}) => {
    ow_1.default(page, ow_1.default.object.validate(validators_1.validators.browserPage));
    ow_1.default(filePath, ow_1.default.string);
...
const blockResources = async (page, resourceTypes = ['stylesheet', 'font', 'image', 'media']) => {
    utils_log_1.default.deprecated('Apify.utils.puppeteer.blockResources() has a high impact on performance in recent versions of Puppeteer. '
        + 'Until this resolves, please use Apify.utils.puppeteer.blockRequests()');
    await puppeteer_request_interception_1.addInterceptRequestHandler(page, async (request) => {
...
const cacheResponses = async (page, cache, responseUrlRules) => {
    ow_1.default(page, ow_1.default.object.validate(validators_1.validators.browserPage));
    ow_1.default(cache, ow_1.default.object);
    ow_1.default(responseUrlRules, ow_1.default.array.ofType(ow_1.default.any(ow_1.default.string, ow_1.default.regExp)));
 ...
 * @name gotoExtended
 */
exports.gotoExtended = async (page, request, gotoOptions = {}) => {
    ow_1.default(page, ow_1.default.object.validate(validators_1.validators.browserPage));
    ...
    ow_1.default(gotoOptions, ow_1.default.object);
    ...
    if (method !== 'GET' || payload || !_.isEmpty(headers)) {
        ...
        const interceptRequestHandler = async (interceptedRequest) => {
            ...
            await puppeteer_request_interception_1.removeInterceptRequestHandler(page, interceptRequestHandler);
            interceptedRequest.continue(overrides);
        };
        await puppeteer_request_interception_1.addInterceptRequestHandler(page, interceptRequestHandler);
    }
 ...
 * @name infiniteScroll
 */
exports.infiniteScroll = async (page, options = {}) => {
    ow_1.default(page, ow_1.default.object.validate(validators_1.validators.browserPage));
    ow_1.default(options, ow_1.default.object.exactShape({
...
const saveSnapshot = async (page, options = {}) => {
    ...
    try {
        const store = await key_value_store_1.openKeyValueStore(keyValueStoreName);
        if (saveScreenshot) {
...
exports.puppeteerUtils = {
    ...
    enqueueLinks: async (...args) => {
        if (logEnqueueLinksDeprecationWarning) {
            utils_log_1.default.warning('Using enqueueLinks() from the Apify.utils.puppeteer namespace is deprecated. '
                + 'Please use the Apify.utils.enqueueLinks().');
            logEnqueueLinksDeprecationWarning = false;
            return enqueue_links_1.enqueueLinks(...args);
        }
    },
