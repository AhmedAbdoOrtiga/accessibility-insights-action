...
const stealth_1 = require("./stealth/stealth"); // eslint-disable-line no-unused-vars,import/named
...
const launchPuppeteerWithProxy = async (puppeteer, opts) => {
    ...
    const parsedProxyUrl = proxy_chain_1.parseUrl(opts.proxyUrl);
    ...
    if (parsedProxyUrl.username || parsedProxyUrl.password) {
        ...
        anonymizedProxyUrl = await proxy_chain_1.anonymizeProxy(opts.proxyUrl);
    }
    opts.args.push(`--proxy-server=${anonymizedProxyUrl || opts.proxyUrl}`);
    const optsForLog = _.omit(opts, LAUNCH_PUPPETEER_LOG_OMIT_OPTS);
    optsForLog.proxyUrl = proxy_chain_1.redactUrl(opts.proxyUrl);
    optsForLog.args = opts.args.slice(0, opts.args.length - 1);
    utils_log_1.default.info('Launching Puppeteer', optsForLog);
    ...
    if (anonymizedProxyUrl) {
        const cleanUp = () => {
            ...
            proxy_chain_1.closeAnonymizedProxy(anonymizedProxyUrl, true)
                .catch((err) => utils_log_1.default.exception(err, 'closeAnonymizedProxy() failed.'));
        };
...
const getPuppeteerOrThrow = (puppeteerModule = 'puppeteer') => {
    if (typeof puppeteerModule === 'object')
...
exports.launchPuppeteer = async (options = {}) => {
    ow_1.default(options, ow_1.default.object.partialShape({
        args: ow_1.default.optional.array.ofType(ow_1.default.string),
        proxyUrl: ow_1.default.optional.string.url,
        puppeteerModule: ow_1.default.optional.any(ow_1.default.string, ow_1.default.object),
    ...
    if (optsCopy.headless == null) {
        optsCopy.headless = process.env[consts_1.ENV_VARS.HEADLESS] === '1' && process.env[consts_1.ENV_VARS.XVFB] !== '1';
    }
    if (optsCopy.useChrome && (optsCopy.executablePath === undefined || optsCopy.executablePath === null)) {
        optsCopy.executablePath = process.env[consts_1.ENV_VARS.CHROME_EXECUTABLE_PATH] || utils_1.getTypicalChromeExecutablePath();
    }
    ...
    if (optsCopy.proxyUrl) {
        ...
        utils_log_1.default.info('Launching Puppeteer', _.omit(optsCopy, LAUNCH_PUPPETEER_LOG_OMIT_OPTS));
    ...
    // Add stealth
    if (optsCopy.stealth) {
        browser = stealth_1.default(browser, optsCopy.stealthOptions);
    }
    return browser;
};
