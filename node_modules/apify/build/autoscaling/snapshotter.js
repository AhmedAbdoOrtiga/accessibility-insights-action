...
class Snapshotter {
    ...
    async start() {
        ...
        // Start snapshotting.
        this.eventLoopInterval = utilities_1.betterSetInterval(this._snapshotEventLoop.bind(this), this.eventLoopSnapshotIntervalMillis);
        this.clientInterval = utilities_1.betterSetInterval(this._snapshotClient.bind(this), this.clientSnapshotIntervalMillis);
        if (utils_1.isAtHome()) {
            ...
            this.cpuInterval = utilities_1.betterSetInterval(this._snapshotCpuOnLocal.bind(this), this.cpuSnapshotIntervalMillis);
            this.memoryInterval = utilities_1.betterSetInterval(this._snapshotMemoryOnLocal.bind(this), this.memorySnapshotIntervalMillis);
        }
    }
    /**
     ...
     */
    async stop() {
        utilities_1.betterClearInterval(this.eventLoopInterval);
        utilities_1.betterClearInterval(this.memoryInterval);
        utilities_1.betterClearInterval(this.cpuInterval);
        utilities_1.betterClearInterval(this.clientInterval);
    ...
    async _snapshotMemoryOnLocal(intervalCallback) {
        try {
            const now = new Date();
            const memInfo = await utils_1.getMemoryInfo();
    ...
    async _ensureCorrectMaxMemory() {
        if (this.maxMemoryBytes)
            return;
        const { totalBytes } = await utils_1.getMemoryInfo();
        if (utils_1.isAtHome()) {
