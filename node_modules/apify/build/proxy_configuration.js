...
const regexs_1 = require("apify-shared/regexs");
...
class ProxyConfiguration {
    ...
    constructor(options = {}) {
        ow_1.default(options, ow_1.default.object.exactShape({
            groups: ow_1.default.optional.array.ofType(ow_1.default.string.matches(regexs_1.APIFY_PROXY_VALUE_REGEX)),
            apifyProxyGroups: ow_1.default.optional.array.ofType(ow_1.default.string.matches(regexs_1.APIFY_PROXY_VALUE_REGEX)),
            ...
            useApifyProxy: ow_1.default.optional.boolean,
        }));
        const { groups = [], apifyProxyGroups = [], countryCode, apifyProxyCountry, proxyUrls, password = process.env[consts_1.ENV_VARS.PROXY_PASSWORD], newUrlFunction, } = options;
        const groupsToUse = groups.length ? groups : apifyProxyGroups;
        const countryCodeToUse = countryCode || apifyProxyCountry;
        const hostname = process.env[consts_1.ENV_VARS.PROXY_HOSTNAME] || consts_1.LOCAL_ENV_VARS[consts_1.ENV_VARS.PROXY_HOSTNAME];
        const port = Number(process.env[consts_1.ENV_VARS.PROXY_PORT] || consts_1.LOCAL_ENV_VARS[consts_1.ENV_VARS.PROXY_PORT]);
        // Validation
    ...
    newProxyInfo(sessionId) {
        if (typeof sessionId === 'number')
            sessionId = `${sessionId}`;
        ow_1.default(sessionId, ow_1.default.optional.string.maxLength(MAX_SESSION_ID_LENGTH).matches(regexs_1.APIFY_PROXY_VALUE_REGEX));
    ...
    async _setPasswordIfToken() {
        const token = process.env[consts_1.ENV_VARS.TOKEN] || consts_1.LOCAL_ENV_VARS[consts_1.ENV_VARS.TOKEN];
        if (token) {
    ...
    async _fetchStatus() {
        ...
        for (let attempt = 1; attempt <= CHECK_ACCESS_MAX_ATTEMPTS; attempt++) {
            try {
                const response = await utils_request_1.requestAsBrowser(requestOpts);
...
* @function
    */
exports.createProxyConfiguration = async (proxyConfigurationOptions = {}) => {
