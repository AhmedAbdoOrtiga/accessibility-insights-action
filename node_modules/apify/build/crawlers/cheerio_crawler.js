...
const streams_utilities_1 = require("apify-shared/streams_utilities");
...
const utilsRequest = require("../utils_request"); // eslint-disable-line import/no-duplicates
const basic_crawler_1 = require("./basic_crawler"); // eslint-disable-line import/no-duplicates
const utils_log_1 = require("../utils_log");
const crawler_extension_1 = require("./crawler_extension");
...
class CheerioCrawler {
    ...
    constructor(options) {
        ow_1.default(options, ow_1.default.object.exactShape({
            ...
            // Deprecated
            requestOptions: ow_1.default.optional.object,
        }));
        const { handlePageFunction, requestTimeoutSecs = 30, handlePageTimeoutSecs = 60, ignoreSslErrors = true, additionalMimeTypes = [], suggestResponseEncoding, forceResponseEncoding, proxyConfiguration, 
        // Autoscaled pool shorthands
        minConcurrency, maxConcurrency, 
        // Basic crawler options
        requestList, requestQueue, maxRequestRetries, maxRequestsPerCrawl, handleFailedRequestFunction = this._defaultHandleFailedRequestFunction.bind(this), autoscaledPoolOptions = DEFAULT_AUTOSCALED_POOL_OPTIONS, prepareRequestFunction, postResponseFunction, useSessionPool = false, sessionPoolOptions = {}, persistCookiesPerSession = false, 
        // Deprecated
        requestOptions, } = options;
        this.log = utils_log_1.default.child({ prefix: 'CheerioCrawler' });
        ...
        /** @ignore */
        this.basicCrawler = new basic_crawler_1.default({
            // Basic crawler options.
            requestList,
            requestQueue,
            maxRequestRetries,
            ...
            handleRequestTimeoutSecs: handlePageTimeoutSecs * constants_1.BASIC_CRAWLER_TIMEOUT_MULTIPLIER,
     ...
     */
    use(extension) {
        ow_1.default(extension, ow_1.default.object.instanceOf(crawler_extension_1.default));
    ...
    async _handleRequestFunction(crawlingContext) {
        ...
        crawlingContext.response = await utils_1.addTimeoutToPromise(this._requestFunction({ request, session, proxyUrl }), this.requestTimeoutMillis, `request timed out after ${this.requestTimeoutMillis / 1000} seconds.`);
        ...
        return utils_1.addTimeoutToPromise(this.handlePageFunction(crawlingContext), this.handlePageTimeoutMillis, `handlePageFunction timed out after ${this.handlePageTimeoutMillis / 1000} seconds.`);
    }
    /**
    ...
    async _parseResponse(request, responseStream) {
        ...
        const { type, charset } = utils_1.parseContentTypeFromResponse(responseStream);
        const { response, encoding } = this._encodeResponse(request, responseStream, charset);
        const contentType = { type, encoding };
        if (statusCode >= 500) {
            const body = await streams_utilities_1.readStreamToString(response, encoding);
            ...
            const body = await streams_utilities_1.concatStreamToBuffer(response);
    ...
    _getRequestOptions(request, session, proxyUrl) {
        const mandatoryRequestOptions = {
            ...
            abortFunction: (res) => {
                ...
                const { type } = utils_1.parseContentTypeFromResponse(res);
                if (statusCode === 406) {
        ...
        if (/PATCH|POST|PUT/.test(request.method))
            mandatoryRequestOptions.payload = request.payload;
