...
const autoscaled_pool_1 = require("../autoscaling/autoscaled_pool"); // eslint-disable-line import/no-duplicates
...
class BasicCrawler {
    ...
    constructor(options) {
        ...
        const { requestList, requestQueue, handleRequestFunction, handleRequestTimeoutSecs = 60, handleFailedRequestFunction = ({ request }) => {
            const details = _.pick(request, 'id', 'url', 'method', 'uniqueKey');
            log.error('Request failed and reached maximum retries', details);
        }, maxRequestRetries = 3, maxRequestsPerCrawl, autoscaledPoolOptions = {}, sessionPoolOptions = {}, useSessionPool = false, 
        // AutoscaledPool shorthands
        minConcurrency, maxConcurrency, 
        // internal
        log = utils_log_1.default.child({ prefix: 'BasicCrawler' }), } = options;
    ...
    async _runTaskFunction() {
        ...
        try {
            await utils_1.addTimeoutToPromise(this.handleRequestFunction(crawlingContext), this.handleRequestTimeoutMillis, `handleRequestFunction timed out after ${this.handleRequestTimeoutMillis / 1000} seconds.`);
    ...
    async _requestFunctionErrorHandler(error, crawlingContext, source) {
        ...
        if (!request.noRetry && request.retryCount < this.maxRequestRetries) {
            request.retryCount++;
            this.log.exception(error, 'handleRequestFunction failed, reclaiming failed request back to the list or queue', _.pick(request, 'url', 'retryCount', 'id'));
