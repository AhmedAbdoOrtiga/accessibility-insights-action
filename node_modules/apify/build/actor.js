...
exports.getEnv = () => {
    ...
    const envVars = {};
    _.mapObject(consts_1.ENV_VARS, (fullName, shortName) => {
        const camelCaseName = utils_1.snakeCaseToCamelCase(shortName);
        let value = env[fullName];
        // Parse dates and integers.
        if (value && fullName.endsWith('_AT'))
            value = tryParseDate(value);
        else if (_.contains(consts_1.INTEGER_ENV_VARS, fullName))
...
exports.main = (userFunc) => {
    ...
    utils_1.logSystemInfo();
    // Log warning if SDK is outdated.
    utils_1.printOutdatedSdkWarning();
    ...
    const exitWithError = (err, exitCode, message) => {
        utils_log_1.default.exception(err, message);
        exited = true;
    ...
    const run = async () => {
        ...
        try {
            ...
            clearInterval(intervalId);
            if (!exited) {
                exitWithError(err, constants_1.EXIT_CODES.ERROR_USER_FUNCTION_THREW, 'The function passed to Apify.main() threw an exception:');
            }
        }
    };
    run().catch((err) => {
        exitWithError(err, constants_1.EXIT_CODES.ERROR_UNKNOWN, 'Unknown error occurred');
    });
};
/**
 ...
 * @name call
 */
exports.call = async (actId, input, options = {}) => {
    ow_1.default(actId, ow_1.default.string);
    ...
    if (input) {
        callActorOpts.contentType = utils_1.addCharsetToContentType(callActorOpts.contentType);
        input = key_value_store_1.maybeStringify(input, callActorOpts);
    }
    const client = token ? utils.newClient({ token }) : utils_1.apifyClient;
    let run;
    try {
 ...
 * @name callTask
 */
exports.callTask = async (taskId, input, options = {}) => {
    ow_1.default(taskId, ow_1.default.string);
    ow_1.default(input, ow_1.default.optional.any(ow_1.default.string, ow_1.default.object));
    ow_1.default(options, ow_1.default.object.exactShape({
        token: ow_1.default.optional.string,
        memoryMbytes: ow_1.default.optional.number.not.negative,
        timeoutSecs: ow_1.default.optional.number.not.negative,
 ...
 * @name metamorph
 */
exports.metamorph = async (targetActorId, input, options = {}) => {
    ow_1.default(targetActorId, ow_1.default.string);
    // input can be anything, no reason to validate
    ow_1.default(options, ow_1.default.object.exactShape({
        contentType: ow_1.default.optional.string.nonEmpty,
        build: ow_1.default.optional.string,
    ...
    if (input) {
        metamorphOpts.contentType = utils_1.addCharsetToContentType(metamorphOpts.contentType);
        input = key_value_store_1.maybeStringify(input, metamorphOpts);
    }
    ...
    await utils_1.sleep(customAfterSleepMillis || METAMORPH_AFTER_SLEEP_MILLIS);
};
/**
 *
 ...
 * @name addWebhook
 */
exports.addWebhook = async (options) => {
    ow_1.default(options, ow_1.default.object.exactShape({
