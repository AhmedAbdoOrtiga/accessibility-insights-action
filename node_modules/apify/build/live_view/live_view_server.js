...
const ensureDir = util_1.promisify(fs.ensureDir);
const LOCAL_STORAGE_DIR = process.env[consts_1.ENV_VARS.LOCAL_STORAGE_DIR] || '';
...
class LiveViewServer {
     ...
     */
    constructor(options = {}) {
        const { screenshotDirectoryPath = DEFAULT_SCREENSHOT_DIR_PATH, maxScreenshotFiles = 10, snapshotTimeoutSecs = 3, maxSnapshotFrequencySecs = 2, } = options;
        this.log = utils_log_1.default.child({ prefix: 'LiveViewServer' });
    ...
    async start() {
        ...
        try {
            ...
            await utilities_1.promisifyServerListen(this.httpServer)(this.port);
    ...
    async stop() {
        ...
        return new Promise((resolve) => {
            this.httpServer.close((err) => {
                this._isRunning = false;
                if (err)
                    this.log.exception(err, 'Live view web server could not be stopped.');
                else
                    this.log.info('Live view web server stopped.');
                resolve();
            });
        });
    }
    ...
    async serve(page) {
        ...
        if (this.lastSnapshot) {
            if (this.lastSnapshot.age() < this.maxSnapshotFrequencyMillis)
                return;
        }
        try {
            this.servingSnapshot = true;
            const snapshot = await utils_1.addTimeoutToPromise(this._makeSnapshot(page), this.snapshotTimeoutMillis, 'LiveViewServer: Serving of Live View timed out.');
    ...
    async _makeSnapshot(page) {
        ...
        const snapshot = new snapshot_1.default({ pageUrl, htmlContent, screenshotIndex });
    ...
    _setupHttpServer() {
        const containerPort = process.env[consts_1.ENV_VARS.CONTAINER_PORT] || consts_1.LOCAL_ENV_VARS[consts_1.ENV_VARS.CONTAINER_PORT];
        this.port = parseInt(containerPort, 10);
        if (!(this.port >= 0 && this.port <= 65535)) {
            throw new Error('Cannot start LiveViewServer - invalid port specified by the '
                + `${consts_1.ENV_VARS.CONTAINER_PORT} environment variable (was "${containerPort}").`);
        }
        this.liveViewUrl = process.env[consts_1.ENV_VARS.CONTAINER_URL] || consts_1.LOCAL_ENV_VARS[consts_1.ENV_VARS.CONTAINER_URL];
...
exports.default = LiveViewServer;
