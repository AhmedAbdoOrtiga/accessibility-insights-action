...
const convertRelativeImagePathsToAbsoluteInReadme = ({
  ...
  const branchName = parsedRepoUrl.hash || gitBranchName || 'master'; // Images in markdown have syntax ![alt text](image url)

  const relativeImageMarkdownRegex = /(!\[.*?\])\(\.\/(.*?)\)/g; // HTML image references of type <img src="..." /> can be also embedded in markdown (e.g. in HTML table)
