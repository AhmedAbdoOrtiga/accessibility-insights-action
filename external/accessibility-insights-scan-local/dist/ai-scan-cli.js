...
const report_name_generator_1 = __webpack_require__(/*! ./report/report-name-generator */ "./src/report/report-name-generator.ts");
const crawler_command_runner_1 = __webpack_require__(/*! ./runner/crawler-command-runner */ "./src/runner/crawler-command-runner.ts");
const url_command_runner_1 = __webpack_require__(/*! ./runner/url-command-runner */ "./src/runner/url-command-runner.ts");
class CliEntryPoint {
    constructor(container) {
        this.container = container;
    }
    async runScan(scanArguments) {
        try {
            const commandRunner = this.getCommandRunner(scanArguments);
            await commandRunner.runCommand(scanArguments);
        }
        catch (error) {
            const reportDiskWriter = this.container.get(report_disk_writer_1.ReportDiskWriter);
            const reportNameGenerator = this.container.get(report_name_generator_1.ReportNameGenerator);
            const errorLog = reportDiskWriter.writeToDirectory(scanArguments.output, reportNameGenerator.generateName('ai-cli-errors', new Date()), 'log', `${common_1.System.serializeError(error)}`);
            console.log(error.message);
            console.log(`Something went wrong. Please try again later. If this persists, search for a known issue or file a new one at https://github.com/microsoft/accessibility-insights-service/issues.`);
            console.log(`Error log was saved as ${errorLog}`);
        }
    }
    getCommandRunner(scanArguments) {
        if (!scanArguments.crawl && lodash_1.isEmpty(scanArguments.inputFile) && lodash_1.isEmpty(scanArguments.inputUrls)) {
            return this.container.get(url_command_runner_1.UrlCommandRunner);
        }
        else {
            return this.container.get(crawler_command_runner_1.CrawlerCommandRunner);
        }
    }
}
...
const lodash_1 = __webpack_require__(/*! lodash */ "lodash");
const yargs = __importStar(__webpack_require__(/*! yargs */ "yargs"));
const common_1 = __webpack_require__(/*! common */ "../common/dist/index.js");
const cli_entry_point_1 = __webpack_require__(/*! ./cli-entry-point */ "./src/cli-entry-point.ts");
const setup_cli_container_1 = __webpack_require__(/*! ./setup-cli-container */ "./src/setup-cli-container.ts");
(async () => {
    const scanArguments = getScanArguments();
    const cliEntryPoint = new cli_entry_point_1.CliEntryPoint(setup_cli_container_1.setupCliContainer());
    await cliEntryPoint.runScan(scanArguments);
})().catch((error) => {
    console.log('Exception occurred while running the tool: ', common_1.System.serializeError(error));
    process.exitCode = 1;
});
function getScanArguments() {
    ...
    return yargs
        ...
        .check((args) => {
        if (args.crawl && lodash_1.isEmpty(args.url)) {
            throw new Error('The --url option is required for website crawling.');
        }
        if (lodash_1.isEmpty(args.url) && lodash_1.isEmpty(args.inputFile) && lodash_1.isEmpty(args.inputUrls)) {
...
/***/ "./src/crawler-parameters-builder.ts":
/*!*******************************************!*\
  !*** ./src/crawler-parameters-builder.ts ***!
  \*******************************************/
/*! no static exports found */
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.CrawlerParametersBuilder = void 0;
// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
const fs_1 = __importDefault(__webpack_require__(/*! fs */ "fs"));
const inversify_1 = __webpack_require__(/*! inversify */ "inversify");
const common_1 = __webpack_require__(/*! common */ "../common/dist/index.js");
let CrawlerParametersBuilder = class CrawlerParametersBuilder {
    constructor(urlObj = common_1.Url, fileSystem = fs_1.default) {
        this.urlObj = urlObj;
        this.fileSystem = fileSystem;
    }
    build(scanArguments) {
...
CrawlerParametersBuilder = __decorate([
    inversify_1.injectable(),
    __metadata("design:paramtypes", [Object, Object])
], CrawlerParametersBuilder);
exports.CrawlerParametersBuilder = CrawlerParametersBuilder;
...
/***/ "./src/report/report-disk-writer.ts":
/*!******************************************!*\
  !*** ./src/report/report-disk-writer.ts ***!
  \******************************************/
/*! no static exports found */
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReportDiskWriter = void 0;
// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
const fs = __importStar(__webpack_require__(/*! fs */ "fs"));
const path = __importStar(__webpack_require__(/*! path */ "path"));
const filenamify_1 = __importDefault(__webpack_require__(/*! filenamify */ "filenamify"));
const filenamify_url_1 = __importDefault(__webpack_require__(/*! filenamify-url */ "filenamify-url"));
const inversify_1 = __webpack_require__(/*! inversify */ "inversify");
const lodash_1 = __webpack_require__(/*! lodash */ "lodash");
const normalize_path_1 = __importDefault(__webpack_require__(/*! normalize-path */ "normalize-path"));
/* eslint-disable security/detect-non-literal-fs-filename */
let ReportDiskWriter = class ReportDiskWriter {
    constructor(fileSystemObj = fs, pathObj = path) {
        this.fileSystemObj = fileSystemObj;
        this.pathObj = pathObj;
    }
    writeToDirectory(directory, fileName, format, content) {
        let reportFileName;
        try {
            reportFileName = `${filenamify_url_1.default(fileName, { replacement: '_' })}.${format}`;
        }
        catch {
            reportFileName = `${filenamify_1.default(fileName, { replacement: '_' })}.${format}`;
        }
        const normalizedDirectory = this.ensureDirectory(directory);
        const filePath = normalize_path_1.default(this.pathObj.resolve(normalizedDirectory, reportFileName));
        this.fileSystemObj.writeFileSync(filePath, content);
        return filePath;
    }
    ensureDirectory(directory) {
        let normalizedDirectory;
        if (lodash_1.isEmpty(directory)) {
            normalizedDirectory = normalize_path_1.default(__dirname);
        }
        else {
            normalizedDirectory = normalize_path_1.default(directory);
        }
...
ReportDiskWriter = __decorate([
    inversify_1.injectable(),
    __metadata("design:paramtypes", [Object, Object])
], ReportDiskWriter);
exports.ReportDiskWriter = ReportDiskWriter;
...
const service_name_1 = __webpack_require__(/*! ../service-name */ "./src/service-name.ts");
let ReportGenerator = class ReportGenerator {
    constructor(reporterFactoryFunc, axeInfo) {
        this.reporterFactoryFunc = reporterFactoryFunc;
        this.axeInfo = axeInfo;
    }
    generateReport(axeResults) {
        ...
        const htmlReportParams = {
            ...
            serviceName: service_name_1.serviceName,
            scanContext: {
    ...
    async generateSummaryReport(scanDetails, results, userAgent) {
    ...
    compareScanResult(result1, result2) {
...
ReportGenerator = __decorate([
    ...
    __metadata("design:paramtypes", [typeof (_a = typeof accessibility_insights_report_1.ReporterFactory !== "undefined" && accessibility_insights_report_1.ReporterFactory) === "function" ? _a : Object, typeof (_b = typeof axe_info_1.AxeInfo !== "undefined" && axe_info_1.AxeInfo) === "function" ? _b : Object])
], ReportGenerator);
exports.ReportGenerator = ReportGenerator;
...
/***/ "./src/report/report-name-generator-builder.ts":
/*!*****************************************************!*\
  !*** ./src/report/report-name-generator-builder.ts ***!
  \*****************************************************/
/*! no static exports found */
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReportNameGeneratorBuilder = void 0;
// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
const inversify_1 = __webpack_require__(/*! inversify */ "inversify");
const lodash_1 = __webpack_require__(/*! lodash */ "lodash");
let ReportNameGeneratorBuilder = class ReportNameGeneratorBuilder {
    getDateSegment(scanDate) {
        return `${scanDate.getFullYear()}${this.padStartWithZero(scanDate.getMonth() + 1, 2)}${this.padStartWithZero(scanDate.getDate(), 2)}`;
    }
    getTitleSegment(pageTitle) {
        return Array.from(pageTitle).filter(this.isValidCharForTitle).slice(0, 20).join('');
    }
    getTimeSegment(scanDate) {
        return (this.padStartWithZero(scanDate.getHours(), 2) +
            this.padStartWithZero(scanDate.getMinutes(), 2) +
            this.padStartWithZero(scanDate.getSeconds(), 2));
    }
    padStartWithZero(num, digits) {
        return lodash_1.padStart(num.toString(), digits, '0');
    }
    isValidCharForTitle(character) {
        return /[A-Za-z0-9]/.test(character);
    }
};
ReportNameGeneratorBuilder = __decorate([
    inversify_1.injectable()
], ReportNameGeneratorBuilder);
exports.ReportNameGeneratorBuilder = ReportNameGeneratorBuilder;


/***/ }),

/***/ "./src/report/report-name-generator.ts":
/*!*********************************************!*\
  !*** ./src/report/report-name-generator.ts ***!
  \*********************************************/
/*! no static exports found */
...
var _a;
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReportNameGenerator = void 0;
// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
const inversify_1 = __webpack_require__(/*! inversify */ "inversify");
const report_name_generator_builder_1 = __webpack_require__(/*! ./report-name-generator-builder */ "./src/report/report-name-generator-builder.ts");
let ReportNameGenerator = class ReportNameGenerator {
    constructor(reportNameGeneratorBuilder) {
        this.reportNameGeneratorBuilder = reportNameGeneratorBuilder;
    }
    generateName(baseName, scanDate) {
        return `${baseName}-${this.reportNameGeneratorBuilder.getDateSegment(scanDate)}-${this.reportNameGeneratorBuilder.getTimeSegment(scanDate)}`;
    }
};
ReportNameGenerator = __decorate([
    inversify_1.injectable(),
    __param(0, inversify_1.inject(report_name_generator_builder_1.ReportNameGeneratorBuilder)),
    __metadata("design:paramtypes", [typeof (_a = typeof report_name_generator_builder_1.ReportNameGeneratorBuilder !== "undefined" && report_name_generator_builder_1.ReportNameGeneratorBuilder) === "function" ? _a : Object])
], ReportNameGenerator);
exports.ReportNameGenerator = ReportNameGenerator;


/***/ }),

/***/ "./src/runner/crawler-command-runner.ts":
/*!**********************************************!*\
  !*** ./src/runner/crawler-command-runner.ts ***!
  \**********************************************/
/*! no static exports found */
...
var _a, _b, _c, _d;
Object.defineProperty(exports, "__esModule", { value: true });
exports.CrawlerCommandRunner = void 0;
// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
const fs = __importStar(__webpack_require__(/*! fs */ "fs"));
const inversify_1 = __webpack_require__(/*! inversify */ "inversify");
...
let CrawlerCommandRunner = class CrawlerCommandRunner {
    constructor(crawler, crawlerParametersBuilder, consolidatedReportGenerator, reportDiskWriter, filesystem = fs) {
        ...
        this.consolidatedReportGenerator = consolidatedReportGenerator;
        this.reportDiskWriter = reportDiskWriter;
        this.filesystem = filesystem;
    }
    async runCommand(scanArguments) {
    ...
    canRunCommand(scanArguments) {
        // eslint-disable-next-line security/detect-non-literal-fs-filename
        if (this.filesystem.existsSync(scanArguments.output) && !scanArguments.restart && !scanArguments.continue) {
            console.log('The last scan result was found on a disk. Use --continue option to continue scan for the last URL provided, or --restart option to delete the last scan result.');
            return false;
        }
        return true;
    }
...
/***/ "./src/runner/url-command-runner.ts":
/*!******************************************!*\
  !*** ./src/runner/url-command-runner.ts ***!
  \******************************************/
/*! no static exports found */
...
var _a, _b, _c;
Object.defineProperty(exports, "__esModule", { value: true });
exports.UrlCommandRunner = void 0;
// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
const cli_spinner_1 = __webpack_require__(/*! cli-spinner */ "cli-spinner");
const inversify_1 = __webpack_require__(/*! inversify */ "inversify");
const report_disk_writer_1 = __webpack_require__(/*! ../report/report-disk-writer */ "./src/report/report-disk-writer.ts");
const report_generator_1 = __webpack_require__(/*! ../report/report-generator */ "./src/report/report-generator.ts");
const ai_scanner_1 = __webpack_require__(/*! ../scanner/ai-scanner */ "./src/scanner/ai-scanner.ts");
let UrlCommandRunner = class UrlCommandRunner {
    constructor(scanner, reportGenerator, reportDiskWriter) {
        this.scanner = scanner;
        this.reportGenerator = reportGenerator;
        this.reportDiskWriter = reportDiskWriter;
    }
    async runCommand(scanArguments) {
        const spinner = new cli_spinner_1.Spinner(`Running scanner... %s \t`);
        let axeResults;
        try {
            spinner.start();
...
UrlCommandRunner = __decorate([
    inversify_1.injectable(),
    __param(0, inversify_1.inject(ai_scanner_1.AIScanner)),
    __param(1, inversify_1.inject(report_generator_1.ReportGenerator)),
    __param(2, inversify_1.inject(report_disk_writer_1.ReportDiskWriter)),
    __metadata("design:paramtypes", [typeof (_a = typeof ai_scanner_1.AIScanner !== "undefined" && ai_scanner_1.AIScanner) === "function" ? _a : Object, typeof (_b = typeof report_generator_1.ReportGenerator !== "undefined" && report_generator_1.ReportGenerator) === "function" ? _b : Object, typeof (_c = typeof report_disk_writer_1.ReportDiskWriter !== "undefined" && report_disk_writer_1.ReportDiskWriter) === "function" ? _c : Object])
], UrlCommandRunner);
exports.UrlCommandRunner = UrlCommandRunner;


/***/ }),

/***/ "./src/scan-result-providers/scan-result-reader.ts":
/*!*********************************************************!*\
  !*** ./src/scan-result-providers/scan-result-reader.ts ***!
  \*********************************************************/
/*! no static exports found */
...
AIScanner = __decorate([
    ...
    __metadata("design:paramtypes", [typeof (_a = typeof scanner_global_library_1.Page !== "undefined" && scanner_global_library_1.Page) === "function" ? _a : Object])
], AIScanner);
...
exports.serviceName = 'Accessibility Insights Service';
...
/***/ "filenamify-url":
/*!*********************************!*\
  !*** external "filenamify-url" ***!
  \*********************************/
/*! no static exports found */
/***/ (function(module, exports) {

module.exports = require("filenamify-url");
...
//# sourceMappingURL=ai-scan-cli.js.map